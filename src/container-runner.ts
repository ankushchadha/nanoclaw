...
async function buildContainerArgs(
  ...
  // Upstream opt-in per-container resource knobs (env-driven). --cpus adds CPU
  // tunability; CONTAINER_MEMORY_LIMIT, if set, overrides the 4g hardening default
  // above (Docker last-write-wins). Empty = our hardening default unchanged.
  ...
  // retries.
  // Local customization (use-native-credential-proxy skill): when native
  // credentials are enabled (.env-supplied), skip the gateway entirely.
  if (!nativeCredentialsEnabled()) {
    ...
    // Claude Code only sends Bearer auth when it sees an OAuth-shaped token, so
    // thread a worthless placeholder — the real token never enters the container.
    // Blank ANTHROPIC_API_KEY (Docker last-write-wins) so the x-api-key path can't
    // win the SDK's credential precedence.
    const oauthArgs = oauthGatewayEnvArgs();
    if (oauthArgs.length) {
      args.push(...oauthArgs);
      // OAuth-via-gateway depends on the agent's OneCLI vault having an Anthropic
      // OAuth secret assigned; if not (e.g. a fresh selective-mode agent) the
      // placeholder Bearer 401s silently. Surface it so the gotcha is diagnosable.
      log.info(
        'OAuth-via-gateway: injecting placeholder token — requires an Anthropic OAuth secret assigned to this OneCLI agent',
        { containerName, agent: agentIdentifier },
      );
    }
  } else {
