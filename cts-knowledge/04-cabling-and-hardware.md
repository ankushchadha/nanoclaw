...
## ⚠️ This rig is the LEGACY variant — F1058 governs where it differs from F1034 (Serial)

The rig is a **GEN7-TMR-L = LEGACY** timer, so its manual is **F1058** (Gen7 Legacy Timer User Guide, Rev 202603). Most facts are variant-independent (verified identical in both guides): MM port (USB Type-B → virtual COM), names over RS-485 + "Load Scoreboard Names" + SCB pre-load + the v2023 names floor, UDP port 60287, the Team Scores / Complete Event Results v2026 + v4.7.0 floor, networking (TCP 22 / 7105, UDP 5353, APIPA, "Event Sequence not received"), and the USB-Connect device. **Two areas genuinely DIFFER — for this rig, trust F1058:**

- **Scoreboard outputs.** F1034 (Serial) describes all-RS-485 outputs named "Blue Square / Black Diamond SCBD." The **Legacy (F1058)** instead has a dedicated **RS-232 scoreboard output (±12V, 60 mA) labeled "Legacy Scoreboard"** alongside RS-485 output(s); the back-panel text labels are **"Gen7 Scoreboard Data"** and **"Legacy Scoreboard"** (F1058 calls the unit *"a direct replacement for any existing CTS Timing Console"*). So this rig's **1/4" scoreboard output (in use) is most likely the RS-232 "Legacy Scoreboard" port** driving the existing board — but the 1/4" **connector form factor is UNVERIFIED in BOTH manuals** (their back-panel figures don't state connector shapes), so keep it rig-observed (`07`) and confirm with a clear panel-label photo. (The blue-square/black-diamond markers seen on the rig's panel in photos still need a clear shot to map to the F1058 labels.) **For the live-names upgrade:** the Legacy DOES have RS-485 scoreboard output(s), so the RS-485-to-DL+ names path still applies — just identify the RS-485 output on the panel (it is not "blue/black"-named on the Legacy).
- **Lane-input wiring.** The Serial Gen7 uses serial **nodes / wallplate / chainable serial cable harnesses** (R-015-7xx, red/green keyways) and **"Gen7 Serial Connect Hubs."** The **Legacy does NOT** — it wires touchpads/pushbuttons/plungers **directly via deckplates → primary/backup near/far harnesses → 8 fixed back-panel connectors (98 inputs)**, with the start system on a deckplate or the harness **start-pod** (R-INF-SJ / R-LSI / R-SJ cables). This matches this rig's start-via-harness wiring (`07`). **Ignore any serial-node / wallplate / "Serial Connect Hubs" guidance for this rig — that is Serial-only.**

Also Serial-only (absent on the Legacy): the **5a–5e Output Course Mappings sub-panel** (pool→connector mapping, including the Blue/Black SCBD mapping); the Legacy's "Input/Output Course Mappings" only sets the Timer Start near/far end. Minor Legacy specs: power input 12V **8 A** max, single PSU **R-920-055**, a user-replaceable **fuse**. [F1058 Rev 202603]
